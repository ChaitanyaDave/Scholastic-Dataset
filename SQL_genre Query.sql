...
where CH1_GENRE is null
group by title
order by nullValues desc

select title, CH1_GENRE, count(CH1_GENRE) as Number_of_rows_with_genre 
INTO #temp2
from [dbo].[Scholastic_Challenge_Dataset]
where title in (select title from #temp1)
and CH1_GENRE is not null
group by CH1_GENRE, title
order by title

Select #temp1.title, #temp2.CH1_GENRE 
INTO #temp3
from #temp1
left join #temp2
on #temp1.title = #temp2.title
where #temp2.CH1_GENRE is not null
...
set CH1_GENRE = (select top 1 CH1_GENRE from #temp3 where #temp3.title = @title_name order by title)
